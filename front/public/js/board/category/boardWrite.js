...
mainCategoryBox.addEventListener('change', (e) => {
    ...
    for (i = 0; i <= MainCategory.length - 1; i++) {
        if (MainCategory[i].value === e.target.value) {
            const selectSubCategory = document.querySelectorAll(
                `.subject > div > .${MainCategory[i].value}`
            )
...
const addHashtag = (e) => {
    if (e.keyCode === 13) {
        ...
        hashtagSpan.setAttribute(
            'style',
            'float:left; margin-right:1.5rem; color:grey; cursor: pointer;'
        )
...
const submitHandler = async (e) => {
    ...
    const insertBoardContent = await axios.post('http://3.35.9.237:3000/board/write', body, {
        header: {
