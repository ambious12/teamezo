...
for (let i = 0; i < Object.values(updateBtn).length; i++) {
    ...
    Object.values(updateBtn)[i].addEventListener('click', (e) => {
        location.href = `http://3.35.9.237:3005/board/view?id=${idx}`
    })
}
...
for (let i = 0; i < Object.values(pageNumber).length; i++) {
    ...
    Object.values(pageNumber)[i].addEventListener('click', (e) => {
        location.href = `http://3.35.9.237:3005/admin/board?page=${number}&order=${order}`
    })
}
...
prevBtn.addEventListener('click', (e) => {
    if (pageNum === 1) {
        location.href = `http://3.35.9.237:3005/admin/board?page=${1}&order=${order}`
        alert('첫번째 페이지입니다.')
    } else {
        location.href = `http://3.35.9.237:3005/admin/board?page=${pageNum - 1}&order=${order}`
    }
})
...
for (let i = 0; i <= access.length - 1; i++) {
    ...
    Object.values(access)[i].addEventListener('click', async (e) => {
        ...
        location.href = `http://3.35.9.237:3005/admin/board?page=${pageNum}&order=${order}`
    })
}
...
boardOrderBtn.addEventListener('click', (e) => {
    if (orderWrap.value === 'liked') {
        location.href = `http://3.35.9.237:3005/admin/board?page=${pageNum}&order=liked`
    } else if (orderWrap.value === 'hit') {
        location.href = `http://3.35.9.237:3005/admin/board?page=${pageNum}&order=hit`
    } else {
        return
    }
})
